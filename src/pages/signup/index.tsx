...
const Signup: FC = () => {
		...
		try {
			...
			if (!response.data.error) {
				await swal(
					...
					`Verify your email, we have sent an email to ${email}, you need to verify your email to continue. If the verification email is not in your inbox, please check your Spam folder`,
					"success"
				);
	...
	return (
		<Fragment>
			<Helmet>
				<title>Signup | Scissor | Developed by Franklin Alegu</title>
			</Helmet>
											...
											<div className='flex flex-col gap-8 items-start justify-between md:flex-row md:gap-2'>
												<div className={`flex flex-col gap-1 w-full md:w-fit`}>
													<Field
														...
														className={`${styles.bioInput} p-3 rounded-xl border-2 focus:outline-none focus:border-primary300 lg:p-4`}
													/>
													<ErrorMessage
														className='text-errorRed'
														name='firstName'
